...
// Predefined format options; these can be expanded if new formats are introduced
...
// Monetag links used to monetise downloads. When a user clicks download
// we open one of these links in a new tab before starting the file download. The link
// is chosen at random on each download to distribute impressions across campaigns.
const monetagLinks = [
...
export default function HomePage() {
  ...
  // Derive a featured file with friendly field names if any exist. The first file
  // returned from the database is treated as featured. If there are no files
  // yet, featuredFile remains null and the featured panel is hidden.
  ...
  // Filter files based on search query and selected format
  ...
  // Handle download: choose a random Monetag link, open it in a new tab and then
  // start downloading the file in the current tab. This distributes traffic
  // evenly across multiple campaigns.
  ...
  return (
    <main className="min-h-screen">
        ...
        <div className="grid items-center gap-10 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-slate-300 md:text-lg">
              Browse organised categories like APK, EXE, MSI, ZIP, RAR and PDF. Each file page is built to feel clean,
              modern and easy to use while still being SEO‑friendly.
            </p>
            ...
            {/* Format filters */}
            <div id="formats" className="mt-8 flex flex-wrap gap-3">
          ...
          {featuredFile && (
            <div className="rounded-[32px] border border-white/10 bg-white/[0.04] p-4 shadow-2xl shadow-black/25 backdrop-blur-xl">
              <div className="overflow-hidden rounded-[28px] border border-white/10 bg-slate-900/90">
                ...
                <img src={featuredFile.image} alt={featuredFile.title} className="h-64 w-full object-cover" />
                <div className="p-5">
                  <div className="mb-3 flex flex-wrap items-center gap-2 text-xs text-slate-300">
                    ...
                    <h2 className="text-2xl font-bold">{featuredFile.title}</h2>
                    <p className="mt-3 text-sm leading-6 text-slate-400">{featuredFile.description}</p>
                    <button
                      onClick={() => handleDownload(featuredFile.fileUrl)}
                      className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-violet-600 px-5 py-3 text-sm font-semibold text-white"
                    >
      ...
      <section className="mx-auto max-w-7xl px-6 pb-6">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {categoryCards.map((item) => {
            ...
            return (
              <div key={item.title} className="rounded-[24px] border border-white/10 bg-white/[0.04] p-5 shadow-xl shadow-black/10">
      ...
      <section id="downloads" className="mx-auto max-w-7xl px-6 py-16">
        <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold">All Categories and File Formats</h2>
          </div>
          <p className="max-w-2xl text-sm leading-6 text-slate-400">
            This section is built for dynamic uploads later. You will be able to add file title, image, category, format,
            SEO title, description and real file size from the backend dashboard.
          </p>
        </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-6 py-16 lg:grid-cols-[1fr_0.95fr] lg:items-center">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-slate-300">
              Zipora can introduce users to FluxSave, where they can download Spotify music and videos from platforms like
              YouTube, Instagram, Facebook, Pinterest, X, TikTok, Snapchat and Audiomack in one place.
            </p>
            <div className="mt-6 flex flex-wrap gap-3 text-sm text-slate-300">
              {[
                ...
                <span key={tag} className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1">
                  {tag}
                </span>
              ))}
            </div>
            <a
              ...
              className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-pink-600 to-purple-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-purple-950/30"
            >
              Explore FluxSave
            </a>
          </div>
      ...
      <section id="about" className="mx-auto max-w-7xl px-6 py-16">
        <h2 className="text-3xl font-bold">About Zipora</h2>
        <p className="mt-4 max-w-3xl text-base leading-7 text-slate-400">
          Zipora is a modern download platform designed for students, creators and anyone who needs quick access to
          digital files. Built with performance, clarity and search visibility in mind, it offers clean file pages, rich
          metadata and a simple admin dashboard for adding new uploads. Files can be categorised into APK, EXE, MSI, ZIP,
          RAR and PDF to help users navigate and discover downloads more easily.
        </p>
      </section>
    </main>
  );
